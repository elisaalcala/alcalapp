...
public class TicketServiceImpl implements TicketService {
	...
	public Ticket editTicket(long id, Ticket updatedTicket, Employee employee) {
		...
		if (!StringUtils.isNullOrEmpty(updatedTicket.getStatusTicket())) {
			...
			if (!isCloseOrFinish(updatedTicket) && isCloseOrFinish(ticket)) {
				...
				Message message = messageService.messageReOpen(Timestamp.valueOf(currentDate),
				...
				Message message = messageService.messageFinish(Timestamp.valueOf(currentDate),
						employee.getUserEmployee());
				if (ObjectUtils.isEmpty(ticket.getMessageTicket())) {
					List<Message> mensajes = new ArrayList<>();
					mensajes.add(message);
	...
	private boolean isCloseOrFinish(Ticket ticket) {
		return ticket.getStatusTicket().equalsIgnoreCase(Constants.STATUS_CLOSED)
				|| ticket.getStatusTicket().equalsIgnoreCase(Constants.RESOLVED);
	}
	...
	public List<Ticket> findticketsBlockedByTeam(Team team) {
		return ticketRepository.findByTeamAssignAndStatusTicketIn(team, Arrays.asList(Constants.STATUS_BLOCKED));
	}

	@Override
	public List<Ticket> findTicketsNotCompletedByEmployee(Employee employee) {
		return ticketRepository.findByEmployeeAssignAndStatusTicketIn(employee, Constants.STATUS_NOT_COMPLETED);
	}
	...
	public List<Ticket> findTicketsReadyByEmployee(Employee employee) {
		return ticketRepository.findByEmployeeAssignAndStatusTicketIn(employee, Constants.STATUS_READY);
	}

	@Override
	public List<Ticket> findTicketsReadyByTeam(Team team) {
		return ticketRepository.findByTeamAssignAndStatusTicketIn(team, Constants.STATUS_READY);
	}
	...
	public Ticket mapNewTicket(Ticket updatedTicket, Employee employee, Team team, Team teamAssign, Message message,
			Message messageAssign) {
		...
		updatedTicket.setStatusTicket(Constants.STATUS_BACKLOG);
		List<Message> mensajes = new ArrayList<>();
		mensajes.add(message);
		mensajes.add(messageAssign);
		updatedTicket.setMessageTicket(mensajes);
		save(updatedTicket);
		updatedTicket.setNameTicket(Constants.NAME_TCK + updatedTicket.getIdTicket());
	...
	public Ticket moveTicket(Ticket ticket, Team team, Message message) {
		...
		ticket.setStatusTicket(Constants.STATUS_BACKLOG);
		LocalDateTime currentDate = LocalDateTime.now();
	...
	public List<Ticket> findTicketsFinishByEmployee(Employee employee) {
		return ticketRepository.findByEmployeeAssignAndStatusTicketIn(employee, Constants.STATUS_COMPLETED_TICKET);
	}

	@Override
	public Integer findCountTicketsFinishByEmployee(Employee employee) {
		Integer ticketCount = (int) ticketRepository.countByEmployeeAssignAndStatusTicketIn(employee,
				Constants.STATUS_COMPLETED_TICKET);
		return ticketCount;
	}
		...
		for (Ticket ticket : ticketsFinish) {
			if (ticket.getFinishDate() != null && ticket.getInitialDate() != null) {
				...
				if (!finishDateTime.toLocalDate().isBefore(now.minusMonths(5).withDayOfMonth(1))
					...
					String monthName = finishDateTime.getMonth().getDisplayName(TextStyle.FULL,
							new Locale(Constants.LANGUAGE_ES, Constants.LANGUAGE_ES_UPPER));
		...
		for (Ticket ticket : ticketsCreation) {
			if (ticket.getInitialDate() != null) {
				...
				if (!creationDate.isBefore(now.minusMonths(5).withDayOfMonth(1))
					...
					String monthName = creationDate.getMonth().getDisplayName(TextStyle.FULL,
							new Locale(Constants.LANGUAGE_ES, Constants.LANGUAGE_ES_UPPER));
					monthTicketCountMap.put(monthName, monthTicketCountMap.get(monthName) + 1);
				}
			}
	...
	public String getByEmployeeCreationAndStatusResolvedPerMonth(Employee employee) throws JsonProcessingException {
		return getTickedByEmployeeCreationPerMonth(
				ticketRepository.findByEmployeeCreationAndStatusTicketIn(employee, Arrays.asList(Constants.RESOLVED)));
	}
